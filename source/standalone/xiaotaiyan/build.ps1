...
$run = Join-Path $workspace 'work\taeyeon\weekend-realistic-run'
$frameWork = Join-Path $workspace 'work\taeyeon\standalone-4k'
$archive = Join-Path $project 'frames-4k.zip'
$invuArchive = Join-Path $workspace 'outputs\taeyeon-skins\taeyeon-invu\frames.zip'
...
$icon = Join-Path $project 'xiaotaiyan.ico'
$manifest = Join-Path $project 'app.manifest'
$source = Join-Path $project 'XiaoTaeyeon.cs'
$outputDirectory = Join-Path $workspace 'outputs\taeyeon-two-skins-one-exe'
...
$output = Join-Path $outputDirectory '小泰妍.exe'
...
if (-not (Test-Path -LiteralPath $invuArchive)) { throw "INVU embedded skin archive not found: $invuArchive" }
...
& $compiler `
  ...
  "/resource:$archive,Taeyeon.Standalone.Frames.zip" `
  "/resource:$invuArchive,Taeyeon.Standalone.Invu.Frames.zip" `
  "/win32icon:$icon" `
