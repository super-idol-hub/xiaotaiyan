...
# These rows are deliberate full-row visual replacements approved after the
# original 1x atlas was frozen.  They are still subject to bounds, alignment,
# transparency, and detail checks, but their silhouettes must not be compared
# against the superseded artwork as a release gate.
APPROVED_REPLACEMENT_ROWS = frozenset((0, 9, 10))
...
def place_look_cell(
    ...
    # Normalize every generated direction to the approved neutral height while
    # preserving the source aspect ratio.  Generated coherent rows can differ
    # by a few source pixels in height; a fixed scale would push tall cells into
    # the top edge and make the loop visibly pulse in size.
    height_1x = max(1, round(target.height / SCALE))
    scale_1x = height_1x / crop.height
    width_1x = max(1, round(crop.width * scale_1x))
    left_1x = round(target.lower_center_x / SCALE - local_lower_center * scale_1x)
...
def save_comparison(
    ...
    draw.text((40, 25), "小泰妍清晰度对比", fill=(245, 245, 245), font=title_font)
...
def main() -> None:
    ...
    for (row, column), frame in sorted(frames.items()):
        ...
        if row < 11:
            ...
            if row not in APPROVED_REPLACEMENT_ROWS:
    ...
    manifest = {
        ...
        "baselineQualityComparedRows": [
            row for row in range(11) if row not in APPROVED_REPLACEMENT_ROWS
        ],
        "approvedReplacementRows": sorted(APPROVED_REPLACEMENT_ROWS),
    }
