...
namespace Taeyeon.Standalone
{
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
            try
            {
                ...
                {
                    string reportPath = args.Length > 1 ? args[1] : Path.Combine(Path.GetTempPath(), "taeyeon-self-test.json");
                    string previewPath = args.Length > 2 ? args[2] : Path.Combine(Path.GetTempPath(), "taeyeon-self-test-preview.png");
                ...
                string mutexName = qaMode
                    ? "Local\\TaeyeonStandalonePet_7B9B05F0_QA"
                    : "Local\\TaeyeonStandalonePet_7B9B05F0";
                using (Mutex mutex = new Mutex(true, mutexName, out createdNew))
                {
                    if (!createdNew)
                    {
                        ...
                        MessageBox.Show("小泰妍已经在桌面上啦。", "小泰妍", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return 0;
                    }
    ...
    internal static class CrashReporter
    {
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "taeyeon-pet-error.txt");
    ...
    {
        public const string ResourceName = "Taeyeon.Standalone.Atlas.png";
    ...
    {
        public const string ResourceName = "Taeyeon.Standalone.Frames.zip";
        public const string InvuResourceName = "Taeyeon.Standalone.Invu.Frames.zip";
        ...
        public static byte[] LoadArchiveBytes(string resourceName)
        {
            ...
            {
                throw new ArgumentException("内嵌角色帧资源名不能为空。", "resourceName");
            }

            Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(resourceName);
            if (stream == null)
            {
                throw new InvalidOperationException("找不到内嵌的4K动作帧资源：" + resourceName);
            }
            ...
            try
            {
                if (IsEmbedded)
                {
                    ...
                    if (!IsBuiltIn)
                    {
                        ...
                        if (!FrameResource.ValidateExternalArchive(archiveBytes, out embeddedError))
                        {
                            archiveBytes = null;
                            return false;
                        }
    ...
    internal sealed class SkinCatalog
    {
        ...
        private SkinCatalog(List<SkinPack> packs)
        {
            ...
            _selectionPath = Path.Combine(
                ...
                "PersonalNonCommercialDevelopment",
                "TaeyeonPet",
                "skin.txt");
        }
        ...
        public SkinPack Find(string id)
        {
            return FindById(_packs, id);
        }
    ...
    internal static class SelfTest4K
    {
        public static int Run(string reportPath, string previewPath)
        {
            ...
            byte[] invuArchiveBytes = null;
            int embeddedSkinCount = 0;
            bool embeddedSkinCatalogValid = false;
            ...
            string invuPreviewPath = BuildSiblingPreviewPath(previewPath, "-invu");
            ...
            try
            {
                ...
                SkinCatalog skinCatalog = SkinCatalog.Discover();
                SkinPack builtInSkin = skinCatalog.BuiltIn;
                SkinPack invuSkin = skinCatalog.Find("taeyeon-invu");
                foreach (SkinPack pack in skinCatalog.Packs)
                {
                    if (pack.IsEmbedded)
                    {
                        embeddedSkinCount++;
                ...
                int expectedEmbeddedFrames = 0;
                foreach (int count in FrameResource.UsedCellsPerRow)
                {
                    expectedEmbeddedFrames += count;
                }
                bool invuArchiveLoaded = invuSkin != null
                    && invuSkin.IsEmbedded
                    && !invuSkin.IsBuiltIn
                    && string.Equals(invuSkin.EntryPrefix, string.Empty, StringComparison.Ordinal)
                    && string.Equals(invuSkin.ExclusiveActionName, "月影侧身行礼", StringComparison.Ordinal)
                    && invuSkin.TryLoadArchive(out invuArchiveBytes)
                    && FrameResource.CountPngEntries(invuArchiveBytes) == expectedEmbeddedFrames;
                embeddedSkinCatalogValid = embeddedSkinCount == 2
                    && builtInSkin.IsEmbedded
                    && string.Equals(builtInSkin.Id, "built-in", StringComparison.Ordinal)
                    && invuArchiveLoaded;
                if (!embeddedSkinCatalogValid)
                {
                    errors.Add("the executable does not expose exactly the two required embedded skins");
                }
                else
                {
                    using (Bitmap invuPreview = FrameResource.LoadSourceFrame(
                        invuArchiveBytes,
                        invuSkin.EntryPrefix,
                        (int)PetState.Idle,
                        0))
                    {
                        SavePreview(invuPreview, invuPreviewPath);
                    }
                }
                ...
                using (ScaledFrameCache cache = new ScaledFrameCache(archiveBytes, "frames/"))
                {
                    ...
                    if (invuArchiveBytes != null)
                    {
                        using (ScaledFrameCache invuCache = new ScaledFrameCache(invuArchiveBytes, string.Empty))
                        {
                            Bitmap transitionOld = cache.Get((int)PetState.HandDance, 0, width, height);
                            Bitmap transitionPose = invuCache.Get(
                                SkinTransitionContract.FinalPoseRow,
                                SkinTransitionContract.FinalPoseColumn,
                                width,
                                height);
                            Bitmap transitionIdle = invuCache.Get((int)PetState.Idle, 0, width, height);
            ...
            WriteReport(
                ...
                invuArchiveBytes == null ? 0 : invuArchiveBytes.Length,
                embeddedSkinCount,
                embeddedSkinCatalogValid,
                invuPreviewPath,
                decodedFrames,
                visiblePixels,
        ...
        private static void WriteReport(
            ...
            string previewPath,
            int archiveBytes,
            int invuArchiveBytes,
            int embeddedSkinCount,
            bool embeddedSkinCatalogValid,
            string invuPreviewPath,
            int decodedFrames,
            long visiblePixels,
            ...
            bool ok)
        {
            ...
            builder.AppendLine("  \"embeddedSkinIds\": [\"built-in\", \"taeyeon-invu\"],");
            builder.AppendLine("  \"embeddedSkinCatalogValid\": " + (embeddedSkinCatalogValid ? "true" : "false") + ",");
            builder.AppendLine("  \"invuFrameArchiveResource\": \"" + FrameResource.InvuResourceName + "\",");
            builder.AppendLine("  \"invuArchiveBytes\": " + invuArchiveBytes + ",");
            builder.AppendLine("  \"invuPreview\": \"" + EscapeJson(Path.GetFullPath(invuPreviewPath)) + "\",");
    ...
    {
        private const string FeedUrl = "https://www.sina.cn/media/5967104936";
        ...
        private string DownloadFeed()
        {
            ...
            request.UserAgent = "TaeyeonPet/1.0 (disabled-by-default public update reminder; 10-minute polling)";
    ...
    internal sealed class PetForm : Form
    {
        ...
        protected override void OnShown(EventArgs eventArgs)
        {
            ...
            if (_qaMode)
            {
                ...
                // path between the two actual archives embedded in this EXE.
                BeginInvoke((MethodInvoker)delegate
                {
                    SkinPack invuSkin = _skinCatalog.Find("taeyeon-invu");
                    if (invuSkin == null || !invuSkin.IsEmbedded)
                    {
                        throw new InvalidOperationException("QA 找不到内嵌 INVU 皮肤。");
                    }
                    SkinPack qaTarget = string.Equals(
                        _currentSkin.Id,
                        invuSkin.Id,
                        StringComparison.OrdinalIgnoreCase)
                        ? _skinCatalog.BuiltIn
                        : invuSkin;
                    SwitchSkin(qaTarget);
                });
            }
            ...
            if (!_qaMode)
            {
                try
                {
                    _trayIcon.ShowBalloonTip(2600, "小泰妍来啦", "双击她会挥手，右键可以切换动作和大小。", ToolTipIcon.Info);
                }
                catch
                {
        ...
        private void BuildMenu()
        {
            ...
            ToolStripMenuItem title = new ToolStripMenuItem("小泰妍");
            title.Enabled = false;
            ...
            _weiboReminderItem = new ToolStripMenuItem("公开动态提醒（开/关）");
            ...
            _weiboCheckNowItem = new ToolStripMenuItem("立即检查公开动态");
            _weiboCheckNowItem.Click += delegate { _weiboMonitor.CheckNow(); };
            _menu.Items.Add(_weiboCheckNowItem);

            _startWithWindowsItem = new ToolStripMenuItem("开机自启");
            ...
            _startWithWindowsItem.Click += delegate
            {
                ...
                string errorMessage;
                if (TrySetStartWithWindows(enable, out errorMessage))
                {
        ...
        private const string StartupRegistryValueName = "TaeyeonStandalonePet";

        private static string BuildUsageText(string fullText, string ignored)
        {
            string result = fullText.Replace(
                "右键：可手动预览待机动作，或调整皮肤、公开动态提醒和设置",
                "右键：可手动预览待机动作，或调整皮肤、公开动态提醒、开机自启和设置");
            const string interfaceMarker = "\n\n【可选皮肤包接口】";
            const string developerMarker = "\n\n开发者：";
            int interfaceStart = result.IndexOf(interfaceMarker, StringComparison.Ordinal);
            if (interfaceStart >= 0)
            {
                int developerStart = result.IndexOf(developerMarker, interfaceStart, StringComparison.Ordinal);
                if (developerStart > interfaceStart)
                {
                    result = result.Remove(interfaceStart, developerStart - interfaceStart);
                }
            }
            return result;
        }
            ...
            try
            {
                using (RegistryKey key = Registry.CurrentUser.OpenSubKey(StartupRegistryPath, false))
                {
                    string value = key == null
                        ? null
                        : key.GetValue(StartupRegistryValueName, null) as string;
        ...
        private static bool TrySetStartWithWindows(bool enable, out string errorMessage)
        {
            try
            {
            ...
            {
                errorMessage = "无法更新开机自启设置：" + exception.Message;
                return false;
            }
        }
