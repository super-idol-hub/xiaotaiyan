...
$previousV3 = Join-Path $release '小泰妍桌宠-v3.0-增强版.exe'
if (Test-Path -LiteralPath $previousV3 -PathType Leaf) {
    Copy-Item -LiteralPath $previousV3 -Destination (Join-Path $history '小泰妍桌宠-v3.0-增强版.exe') -Force
}
$previousV301 = Join-Path $release '小泰妍桌宠-v3.0.1-增强版.exe'
if (Test-Path -LiteralPath $previousV301 -PathType Leaf) {
    Copy-Item -LiteralPath $previousV301 -Destination (Join-Path $history '小泰妍桌宠-v3.0.1-增强版.exe') -Force
}
$previousV302 = Join-Path $release '小泰妍桌宠-v3.0.2-增强版.exe'
if (Test-Path -LiteralPath $previousV302 -PathType Leaf) {
    Copy-Item -LiteralPath $previousV302 -Destination (Join-Path $history '小泰妍桌宠-v3.0.2-增强版.exe') -Force
}
$previousV303 = Join-Path $release '小泰妍桌宠-v3.0.3-增强版.exe'
if (Test-Path -LiteralPath $previousV303 -PathType Leaf) {
    Copy-Item -LiteralPath $previousV303 -Destination (Join-Path $history '小泰妍桌宠-v3.0.3-增强版.exe') -Force
}
$previousV304NamedAsPet = Join-Path $release '小泰妍桌宠-v3.0.4-增强版.exe'
if (Test-Path -LiteralPath $previousV304NamedAsPet -PathType Leaf) {
    Copy-Item -LiteralPath $previousV304NamedAsPet -Destination (Join-Path $history '小泰妍桌宠-v3.0.4-增强版.exe') -Force
}
...
if (Test-Path -LiteralPath $currentLatest -PathType Leaf) {
    ...
    if ($currentLatestVersion -and $currentLatestVersion.StartsWith('3.0.5', [StringComparison]::OrdinalIgnoreCase)) {
        Copy-Item -LiteralPath $currentLatest -Destination (Join-Path $history '小泰妍桌宠-v3.0.5-增强版.exe') -Force
    }
}
